...
with open("quran_urdu.txt", "r", encoding="utf-8") as f:
    quran_urdu = f.read()
    print(quran_urdu[:100])
    print(len(quran_urdu))
    print(set(quran_urdu))
    print(len(set(quran_urdu)))
...
def clean_urdu_text(text):
    """
    Remove garbage characters from Urdu text (keep only meaningful Urdu characters)
    """
    ...
    garbage_chars = {
        ...
        '\u060C': '،', # Arabic comma (normalize)
        '\ufdfb': '',  # Bismillah symbol (ﷻ)
        '\ufdfa': '',  # Islamic symbol (ﷺ)
        '\u061B': '؛', # Arabic semicolon
    ...
    for char, replacement in garbage_chars.items():
...
# Clean the Urdu text
quran_urdu_clean = clean_urdu_text(quran_urdu)

print("Original length:", len(quran_urdu))
print("Cleaned length:", len(quran_urdu_clean))
print("Unique characters before:", len(set(quran_urdu)))
print("Unique characters after:", len(set(quran_urdu_clean)))
print("\nCharacters after cleaning:")
print(sorted(set(quran_urdu_clean)))


# Create vocab from CLEANED text (not raw)
total_char = len(quran_urdu_clean)
chars = sorted(set(quran_urdu_clean))
...
data = torch.tensor(encoder(quran_urdu_clean), dtype=torch.long)
